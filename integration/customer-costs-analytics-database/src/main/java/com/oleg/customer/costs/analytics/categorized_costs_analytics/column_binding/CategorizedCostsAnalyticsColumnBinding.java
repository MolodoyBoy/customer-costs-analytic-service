...
class CategorizedCostsAnalyticsColumnBinding implements ColumnBinding {
    ...
    CategorizedCostsAnalyticsColumnBinding() {
        ...
        fieldBindings.put(CATEGORY_DESCRIPTION, CUSTOMER_COSTS_CATEGORY.DESCRIPTION);
            ...
            .reduce((c1, c2) -> {
                if (!c1.equals(c2)) {
                    throw new IllegalArgumentException(
                        format("Columns not belong to same entity! %s %s", c1.getSimpleName(), c2.getSimpleName())
                    );
                } else {
                    return c1;
                }
    ...
    private void validate() {
        for (CategorizedCostsAnalyticsColumn column : values()) {
            if (!fieldBindings.containsKey(column)) {
                throw new IllegalArgumentException(format("Db column not set for column : %s", column.name()));
            }
        }
    }
